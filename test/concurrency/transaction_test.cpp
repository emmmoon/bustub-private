...
void CheckTxnRowLockSize(Transaction *txn, size_t shared_size, size_t exclusive_size) {
  EXPECT_EQ(txn->GetSharedRowLockSet()->size(), shared_size);
  EXPECT_EQ(txn->GetExclusiveRowLockSet()->size(), exclusive_size);
}
...
TEST_F(TransactionTest, GradingVisibilityTest_TestA) {
  // txn1: Table created with id = 0
  // txn2: delete data with v1 = 233 in txn 2 READ_COMMITTED
  // txn3: scan data expect v1 in range [234] in txn 3 READ_COMMITTED
}
